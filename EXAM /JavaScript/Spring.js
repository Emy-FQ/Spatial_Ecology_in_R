...
var spring_collection = ee.ImageCollection("COPERNICUS/S2_SR_HARMONIZED")
 .filterBounds(AOI)
.filterDate('2023-03-01', '2023-05-31') //meterological spring
.filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 3))
.map(maskS2cloudsSCL);

// Check the number of available images 
print(spring_collection.size()); //there are two images

// Select the bands, otain the median image, scale by 10,000, and clip to the AOI
var processed_image = spring_collection.select(['B4', 'B3', 'B2', 'B5', 'B8'])
  .median()
  .divide(10000)
  .clip(AOI);
...
Map.addLayer(processed_image, {bands: ['B4', 'B3', 'B2'], min: 0, max: 0.3}, 'Lough Neagh Spring RGB');
