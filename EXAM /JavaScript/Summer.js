...
var summer_collection = ee.ImageCollection("COPERNICUS/S2_SR_HARMONIZED")
 .filterBounds(AOI)
.filterDate('2023-06-01', '2023-08-31') //meterological summer
.filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 3))
.map(maskS2cloudsSCL);


// Check the number of available images 
print(summer_collection.size()); //there is only one image

// Get the first (and only) image from the filtered collection
var summer_image = summer_collection.first();

// Select the bands, scale by 10,000, and clip to the AOI
var processed_image = summer_image.select(['B4', 'B3', 'B2', 'B5', 'B8'])
  .divide(10000)
  .clip(AOI);

// Display natural RGB colors
Map.centerObject(AOI, 10);
Map.addLayer(processed_image, {bands: ['B4', 'B3', 'B2'], min: 0, max: 0.3}, 'Lough Neagh Summer RGB');
